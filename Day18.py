...
seen = {}
def min_path(curr_loc, inv, locations, distances, level):
    ...
    if len(remaining_keys) == 0 and inv == sorted(list(locations.keys())):
        return 0, []
    elif len(remaining_keys) == 0:
        raise ValueError("No remaining keys, but not all collected")
    possible = []
    for key in remaining_keys:
        ...
        new_inv.sort()
        child_dist, path = min_path(key, new_inv, locations, distances, level+1)
        new_path = path.copy()
        new_path.insert(0, key)
        possible.append((key_dist + child_dist, new_path.copy()))
        if level == 1:
            print("Found dist of", key_dist+child_dist, "for path", new_path)
...
def part_a():
    ...
    return min_path("start", [], locations, distances, 1)
